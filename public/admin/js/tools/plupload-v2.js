var uploadpl = {
	...
	preview  : function(file,id){
		var tag = '<li id="thumbs-'+ id + '" class="thumb-view col-sm-4"><a href="'+ file.id +'" class="color-red del-preview glyphicon glyphicon-off"></a><b></b></li>';
		var preID = '#' + this.container_id + ' ' + this.filelist;
		var item = $( tag ).prependTo( preID );
		var image = $( new Image() ).appendTo(item);
		var preloader = new mOxie.Image();
		preloader.onload = function() {
			//preloader.downsize( '33.333%', 'auto' );
			image.prop({ "src": preloader.getAsDataURL(),'id':id,'class':'img-preview img-responsive'} );
		};
