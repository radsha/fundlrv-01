...
@section('header')
<a href='{{url('fundpic')}}' ><i class="fa  fa-home  text-Primary" style="font-size:25px"></i></a>
   ไฟล์เอกสาร
 @endsection
@section('content')
<div class="panel panel-default">
  <form class="form-horizontal"  method="get" action="{{url('fundpic')}}">
                      ...
                      @if (count($datag)>0)
                        <?php $i=0;?>
                              ...
                              @foreach ($datag as $datag1 )
                                  <li class="list-inline"><?php $i++;?>{{$i.'.'.$datag1->filename}}</li>
                              @endforeach
                                </ul>
                      @endif
                   </td>
                   <td class="text-center"><a  href='{{url('fundpic/'.$data1->mbid.'?samano='.$data1->samano.'&fundid='.$data1->fundid.'&mbmember='.$data1->mbmember.'&keywords='.Request::input('keywords'))}}'> <i class="fa fa-list-alt"></i> </a>            </td>
           </tr>
         @endif
@endforeach
   </tbody>
