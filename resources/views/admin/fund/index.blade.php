...
@section('header')
<a href='{{url('fund')}}' ><i class="fa  fa-home  text-Primary" style="font-size:25px"></i></a>
              ...
              <tr>
                        ...
                        <td class="text-center">
                          ...
                          @if ($fundtype=='1')
                                ...
                                @if($mbstatus=='0')
                                      <a href='{{url('fund/create?fundid='.$datafund1->fundid.'&mbmember='.$data->member_no)}}'  >{!! App\Fundstatus::getfundstatus($mbstatus) !!}  </a>
                                @else
                                    <?php $mbid=App\Fund::getmbid($datafund1->fundid,$data->member_no)  ?>
                                      <a href='{{url('fund/'.$mbid.'/edit?fundid='.$datafund1->fundid.'&mbmember='.$data->member_no)}}'  >{!! App\Fundstatus::getfundstatus($mbstatus) !!}  </a>
                                @endif

                          @else
                              <?php $mbstatuslink=App\Fund::getapplylink($datafund1->fundidlink,$data->member_no)  ?>
                                @if($mbstatuslink=='1')
                                  <a href='{{ url('fund/'.$datafund1->fundid.'?mbmember='.$data->member_no) }}'  >{!! App\Fundstatus::getfundstatus($mbstatus) !!}  </a>
                                @else
                                  <a href='#' onclick="alert('กรุณาสมัครประเภทสามัญก่อนครับ')" >{!! App\Fundstatus::getfundstatus($mbstatus) !!}  </a>
                                @endif
